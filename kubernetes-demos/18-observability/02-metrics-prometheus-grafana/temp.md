...
**02-prometheus-configmap.yaml** contains `prometheus.yml` — the most
...
**05-node-exporter-daemonset.yaml** is a DaemonSet that applies everything
from the DaemonSet labs, with three critical differences from a standard pod:

```yaml
spec:
  hostPID: true         # See host PID namespace — required for process metrics
  hostNetwork: true     # Use host network namespace — required for real NIC metrics
                        # Side effect: pod IP == node IP, no separate container IP
  containers:
    - name: node-exporter
      image: quay.io/prometheus/node-exporter:v1.10.2
      args:
        - --path.procfs=/host/proc    # Read from mounted host /proc
        - --path.sysfs=/host/sys      # Read from mounted host /sys
        - --path.rootfs=/host         # Root of host filesystem (for disk stats)
        - --collector.filesystem.mount-points-exclude=^/(sys|proc|dev|host|etc)($$|/)
        # Exclude virtual and container filesystems from disk metrics
        # Without this filter, you see hundreds of overlayfs mounts (Docker layers)
        # $$ escapes the $ character in YAML multi-line context
      ports:
        - containerPort: 9100
          hostPort: 9100    # Bind directly to host port — hostNetwork mode
      volumeMounts:
        - name: proc
          mountPath: /host/proc
          readOnly: true
        ...
        - name: host-root
          ...
          # HostToContainer: if new mounts appear on the host, they become
          # visible inside the container — needed for disk stats on dynamic mounts
  volumes:
    - name: proc
      hostPath:
        path: /proc
    ...
    - name: host-root
      hostPath:
        path: /
  # All nodes — including control-plane (needs metrics too):
  tolerations:
    - operator: Exists   # Tolerate ALL taints — ensure every node is monitored
```
...
This ConfigMap adds a second datasource to the existing Grafana deployment.
Grafana auto-provisions datasources from `/etc/grafana/provisioning/datasources/`.
The existing volume in Grafana's Deployment already mounts the `grafana-datasources`
ConfigMap — we update that ConfigMap to add the Prometheus entry:
...
Apply the custom dashboard ConfigMap (5 focused panels matching what this lab teaches):
